...
```
なお、-m以降を省いてコマンドを実行することも可能である。詳細は[Git発展編](./git-advanced.html)を参照のこと。
...
```
また、ここでリモートリポジトリとして「origin」などとして間接的に指定するためには別途設定が必要である（[Git発展編](./git-advanced.html)より「リモートリポジトリを対応させる」参照）。当該のURLを直接指定することも可能である。  
...
## Gitの実用
実際にGitを使っていく上では、これらに加えて幾らかの追加知識があると望ましい。「[Git発展編](./git-advanced.html)」を参照せよ。
