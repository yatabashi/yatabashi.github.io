...
また、これらで始点を指定し、そこから下にディレクトリを辿って行って指定を行うこともできる。階層は`/`で区切られる。次のパスは、筆者のローカル環境での、このページを記述するファイルの位置を示すものの一例である。
```
~/Documents/works/github.io/unix-commands.md
```

応用として、二階層上のディレクトリを指定したい場合は`../..`と指定すればよい。三階層以上の場合も同様である。
