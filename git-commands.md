...
`-A`オプションを指定した場合、**ワークツリー**内の全ての差分が（そのファイルがGit管理下にあるかどうかに関わらず）ステージされる。  
ステージの対象に`.`を指定した場合、**カレントディレクトリ**以下の全ての差分が（そのファイルがGit管理下にあるかどうかに関わらず）ステージされる。
...
参考：[忘れやすい人のための git diff チートシート - Qiita](https://qiita.com/shibukk/items/8c9362a5bd399b9c56be)